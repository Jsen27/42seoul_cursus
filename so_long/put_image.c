...
/*   Updated: 2022/10/03 15:55:11 by sehjung          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
		...
		while (j < len)
		{
			if (var->map[i][j] == '1')
				mlx_put_image_to_window(var->mlx,
					var->win, var->rock, j * 16, i * 16);
			else if (var->map[i][j] == 'P')
				mlx_put_image_to_window(var->mlx,
					var->win, var->player, j * 16, i * 16);
			else if (var->map[i][j] == 'E')
				mlx_put_image_to_window(var->mlx,
					var->win, var->exit, j * 16, i * 16);
			else if (var->map[i][j] == 'C')
				mlx_put_image_to_window(var->mlx,
					var->win, var->apple, j * 16, i * 16);
			j++;
		}
		i++;
	}
...
void	put_image(t_var *var, size_t len)
{
	...
	var->apple = mlx_xpm_file_to_image(var->mlx,
